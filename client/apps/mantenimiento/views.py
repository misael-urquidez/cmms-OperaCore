...
class Index(generic.View):
    """Pantalla principal del modulo Mantenimiento. Consume el api/ por HTTP."""

    template_name = "mantenimiento/index.html"

    def get(self, request):
        response = cache.get("mantenimiento_ping")
        if response is None:
            try:
                response = SESSION.get(f"{API_URL}/ping/", timeout=5).json()
            except requests.exceptions.RequestException:
                response = {"status": "sin conexion con el api"}
            cache.set("mantenimiento_ping", response, PING_TTL)
        return render(request, self.template_name, {"modulo": "Mantenimiento", "api_status": response})
...
#     template_name = "mantenimiento/list.html"
#
#     def get(self, request):
...
#     template_name = "mantenimiento/create.html"
#
#     def get(self, request):
