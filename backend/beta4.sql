...
-- 1) columna TRABAJADOR.foto (ruta dentro de api/media/, igual que
--    REPORTE_FALLA.imagen / ORDEN_MANTENIMIENTO.imagen / HERRAMIENTA.imagen)
--    + usuarios admin/tecnico de todo el equipo.
-- 2) tabla HISTORIAL_ESTADO_MAQUINA — auditoria de cada cambio de
--    Maquina.estado_maquina (automatico o manual). Alimenta REGISTRO_OPS
--    (y por lo tanto MTBF/MTTR/Disponibilidad via los triggers ya
--    existentes en triggers2.sql, que NO cambian). Ver
--    apps/maquinaria/services.py:cambiar_estado_maquina en el codigo.
-- 3) FIX: a la tabla LINEA le faltaba una coma despues de la columna
--    `telefono`, lo cual tronaba el script completo con error de
--    sintaxis. Ya corregido.
--
-- Uso: mysql -u <usuario> -p < beta_full.sql
-- Esto BORRA la base "operacore" y la recrea desde cero con todo.
-- Despues de correr este script, correr triggers2.sql aparte (no
-- cambio, no necesita re-ejecutarse si ya lo tienes corrido, pero si
-- recreaste la BD desde cero si hay que volver a correrlo).
-- =====================================================================
...
CREATE TABLE REFACCION (
    ...
    codigoInventario VARCHAR(30) NULL UNIQUE,
    numeroOrden      VARCHAR(20) NULL UNIQUE,
...
-- Tabla puente N:M — qué tipos de máquina son válidos en cada área.
-- Si un tipo_maquina no aparece aquí en ninguna fila, se considera
-- UNIVERSAL (válido en cualquier área). El trigger de validación
-- (ver triggers2.sql) depende de esta tabla.
CREATE TABLE TIPO_MAQUINA_AREA (
    tipo_maquina    INT NOT NULL,
    area            VARCHAR(10) NOT NULL,
    PRIMARY KEY (tipo_maquina, area),
    CONSTRAINT fk_tma_tipo FOREIGN KEY (tipo_maquina) REFERENCES TIPO_MAQUINA(numeroRegistro),
    CONSTRAINT fk_tma_area FOREIGN KEY (area) REFERENCES AREA(codigo)
) ENGINE=InnoDB;
...
-- ============================================================
-- HISTORIAL_ESTADO_MAQUINA — NUEVA. Auditoria de cada cambio de
-- MAQUINA.estado_maquina, automatico (reporte de falla / orden de
-- mantenimiento / sensor) o manual. Cada cambio es un INSERT nuevo,
-- nunca se hace UPDATE sobre un renglon viejo. Con las fechas de aqui
-- se calculan las horas operadas antes de fallar/deshabilitarse, que
-- alimentan REGISTRO_OPS (y con eso, el MTBF via el trigger
-- tg_actualizar_mtbf_registroops que ya existe en triggers2.sql y no
-- se toca). Va aqui, justo despues de MAQUINA, porque depende de
-- MAQUINA y de EDO_MAQUINA (esta ultima ya existe desde la seccion 1).
-- ============================================================
CREATE TABLE HISTORIAL_ESTADO_MAQUINA (
    numeroRegistro   INT AUTO_INCREMENT PRIMARY KEY,
    maquina          VARCHAR(10) NOT NULL,
    estado_anterior  VARCHAR(5)  NULL,
    estado_nuevo     VARCHAR(5)  NOT NULL,
    fecha            DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    referencia_tipo  VARCHAR(20) NULL,   -- 'reporte_falla' | 'orden_mantenimiento' | 'manual'
    referencia_id    VARCHAR(20) NULL,   -- numeroRegistro o folio, para trazar el porque
    CONSTRAINT fk_hist_maquina FOREIGN KEY (maquina) REFERENCES MAQUINA(codigo),
    CONSTRAINT fk_hist_edo_ant FOREIGN KEY (estado_anterior) REFERENCES EDO_MAQUINA(codigo),
    CONSTRAINT fk_hist_edo_nue FOREIGN KEY (estado_nuevo) REFERENCES EDO_MAQUINA(codigo)
) ENGINE=InnoDB;
...
ADD titulo VARCHAR(100) AFTER folio; 
...
CREATE INDEX idx_hist_maquina_fecha ON HISTORIAL_ESTADO_MAQUINA(maquina, fecha);
...
-- Opción A (segura, sin tocar seed data existente): autoriza los 5 tipos
-- SMT usados hoy en el seed (1,2,3,4,6) únicamente en ARR001, que es donde
-- viven LI001/LI002 en los datos de prueba actuales. Transportador (5) no
-- aparece aquí a propósito: al no tener restricción, queda UNIVERSAL
-- (válido en cualquier área).
INSERT INTO TIPO_MAQUINA_AREA (tipo_maquina, area) VALUES
(1, 'ARR001'),  -- Pick & Place
(2, 'ARR001'),  -- Horno Reflow
(3, 'ARR001'),  -- AOI
(4, 'ARR001'),  -- Dispensador de pasta
(6, 'ARR001');  -- Estación de prueba
